  ...
  function logistics(root, arg) {
    ...
    function render() {
      root.innerHTML = `
        ...
        ${tab === 'expedition' ? expPanel() : tab === 'dock' ? dockPanel() : tab === 'supply' ? supplyPanel() : practicePanel()}`;
        ...
        ${EXPEDITIONS.map(ex => {
          ...
          return `<div class="panel" style="margin:6px 0">
            <div class="flex" style="justify-content:space-between;align-items:center">
              ...
              ${active
                ? (left > 0 ? `<span>${UI.countdown(left)}</span>` : `<button class="btn btn-gold btn-sm" data-claimex>完成！领取</button>`)
    ...
    function dockPanel() {
      ...
      return `<div class="panel">
        <h3>入渠修理</h3>
        <div class="section-title">修理船坞</div>
        <div class="flex" style="gap:8px">
          ${st.repairs.map((r, i) => {
            if (!r) return `<div class="fleet-slot" style="width:23%;min-height:120px">船坞${i + 1}<br><span class="dim">空闲</span></div>`;
            const s = st.ships[r.ship];
            const def = Game.shipDef(s);
            const left = r.end - Date.now();
            return `<div class="ship-card" style="width:23%">${UI.portraitImg(s.id, 'portrait')}
              <div class="card-info"><span>${UI.esc(def.zh)}</span><span class="lv">${UI.countdown(left)}</span></div>
              <button class="btn btn-red btn-sm" style="width:100%;border-radius:0" data-cancel="${i}">终止</button></div>`;
          }).join('')}
        </div>
        <div class="section-title">待修理舰娘（点击送入船坞）</div>
        <div class="flex" style="gap:8px">
          ${damaged.map(s => {
            const def = Game.shipDef(s);
      ...
      root.querySelectorAll('[data-cancel]').forEach(b => {
        b.addEventListener('click', () => {
          Logistics.cancelRepair(parseInt(b.dataset.cancel, 10));
          Game.save(); render();
        });
      });
