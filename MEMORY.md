...
## V0.306 海域难度评级（2026-09-16 批次1）—— 新增的长期约定

- **25 张图的难度是数据资产，不是文案**。`maps[].diff`(1~5) + `maps[].diffNote` 是游戏内**唯一**难度数据源，
  由 `scripts/simulate.js` 的「V0.306·批次1 海域难度评级」段锁死：档位完整性 / `diffNote` **必须含至少一个阿拉伯数字**
  （坑 #53）/ `diff` 与 `diffNote` 成对 / **档位分布写死为"设计要求"**（T1:13 · T2:4 · T3:4 · T4:3 · T5:1）——
  动任何一张图的档位都会红，必须**同步改任务书 + 评估报告 + 断言**三处。
- **档位阈值**（`scripts/map_difficulty.js::tierOf`，任务书 1.1 指定，不得改）：Lv15 失败率
  `<3%` T1 / `3~8%` T2 / `8~20%` T3 / `20~40%` T4 / `≥40%` T5。复算命令 **`npm run diff:maps`**。
  该工具只做复算与回归，**改了它会连带作废 `maps[].diff`**。
- **方向性护栏必须带例外表**（坑 #52）：要求「任何图的 `diff` ≥ 其 `need` 前置图的 `diff`」，
  但**既有图 3-5 就是违反者**（3-5 = T1，前置 3-4 = T4）—— `need` 是**解锁前置**，不是难度顺序，
  两者本就可分离（3-5 是 EO 图：血条 7 / 无栖姬 / Lv15 失败率 0%）。
  处置 = `DIFF_GUARD_EXCEPTIONS` 逐条写理由 + **两侧断言**（未登记违规 = 0；已登记项必须**仍然真的违规**，防例外表腐烂）。
  **加新图时不许往例外表里塞条目** —— 那是唯一会让护栏失效的路径；新图真的更简单就说明档位错了，该改的是档位。
- **`stars` 字段保留但不再渲染**（坑 #54）：`.map-stars` 的调用点（常规列表 · 海域详情 · 出击前页头 · 战役详情）
  全部换成 `.diff-badge`，CSS 规则保留以便回退。**不得重新引入第二个难度指标**；
  E2E 有 `diff:noMapStars` / `diff:activeNoMapStars` / `hist:noMapStars` 三条守着。
- **UI 只做"档位→人话"**：`ui/sortie.js` 的 `DIFF_ZH` + `diffBadge(m, withLabel)` / `diffNoteHtml(m)`；
  `diffNote` 走 **`briefHtml()`** 渲染（与作战简报同一套转义 + `**强调**`），**别裸 replace**。
- **`diffNote` 有两条被改写的文案**（等价改写，非重新评估）：1-1「两艘」→「2 艘」；2-1 草案整句无数字 → 补「血条 4 格」。
  原因都是原草案过不了坑 #53 的「必须含阿拉伯数字」断言。**待用户追认**。

## V0.306 批次2 支援舰队 / 批次3 运输作战 + 2026-09-18 评审整改 —— 新增的长期约定

- **先测量后定价**：支援的系数/艘数/消耗**不由人定**，由 `scripts/sim_support.js` 实测回调；
  冻结值写进 `battle.js::SUPPORT_COEF / SUPPORT_MIN_SHIPS / SUPPORT_MAX_SHIPS / SUPPORT_MODE`
  与 `sortie.js::SUPPORT_COST`。改动这些常量**必须重跑 `sim_support.js` 并更新定价报告**（断言锁着常量值）。
  定价方法（`design/支援舰队定价实测_V0.306.md`）：**先给噪声底**（同一条件跑两遍）再读 Δ ——
  N=400 时通过率差的噪声底可达 ±7pp，**只报 Δ 不报噪声底 = 不可读**。
- **零随机数开关是硬约定**：`opts.support` 假值 → 支援阶段整体跳过且**一次 `Math.random` 都不消费**。
  证据 = `drift:v0305` 与 V0.305 基线 **sha256 字节相同**。新增随机数消费者时照抄这套。
- **⚠️ 断言"可达"必须走真实入口，禁止拿数据表自证**（本版 P0 教训）：
  运输点断言原写成 `m.edges.some(e => e.includes('T'))` —— **查到了边，就判"可达"**，
  而真实路由 `moveToNext()` 恒取 `next[0]`，`T` 永远排在 `[C,T]` 第 2 位 ⇒ 玩家一次也碰不到。
  **新节点类型的"可达性"断言一律要跑 `Sortie.start → moveToNext` 的真实路径。**
- **新节点类型清单要含"路由/选择点"**：开工前那份"要动 8~10 处"的清单（`nodes`/`edges`/`defs` →
  `advance()` 分支 → UI 四处 + 出现物品 + 节点提示行）**漏了"节点怎么被选中"**。
  加节点必须同时问：**它在自动选路里什么时候会被选到？分支命中/未命中分别是哪条路？**
- **`mapProgress[m.id]` 的形状只有一个定义点**：`state.js::mapProgressEntry(m)`。
  `newGame()` / `normalizeSave()` / 引擎运行时兜底三条路径都调它。
  **新增字段只改这里** —— 否则会出现"运行时多一个未登记的键，迁移档与新建档形状分叉"。
  （`normalizeSave` 只允许补 `mapProgress` 内的键；版本级新键一律由迁移器补，坑 #30。）
- **一次性奖励的账本项与它守护的进度同生共死**：运输用 `mapProgress[id].transportRewarded`（存时间戳）
  而不是另开 `stats` 账本 —— 若图条目被重建，进度归零、账本也归零，玩家重做即可再领；
  反过来若账本单独存活而进度被清，会「做满门槛却什么都不发」的静默损失。
- **门槛类数值必须配"两条对照"的硬验收**：常规最优编成输送量 **0 < 门槛**，
  专门运输编成（AV 10 + 舟艇 8）**≥ 门槛**。只写 `goal > 0` 是同义反复（裸舰容量恒 0）。
- **运输门槛 18 依赖 `m4a1` 可达**：它是全装备表**唯一**的「上陆用舟艇」，挂在 **2-1 通关任务奖励**
  （`quests.js` o18）。**砍它的渠道 = 砍运输的一条能力线**，两者是耦合的，改动时必须同时想。
- **⚠️ 不存在"不可判定"的统计断言**：`simulate.js` 原有的「史实加成方向：60 场 S 率对照」实测
  **假失败率约 78%** —— 该场景**没有裕度**（我方零沉船、敌方输出微不足道），5% 命中/回避乘区的
  效应量小于噪声底（N=300 时 ΔS 仍正负翻转）。已换成**确定性判据**（读命中乘区纯函数 + 断言乘区只挂我方）。
  **教训：写方向性断言前先证明该场景对该机制有分辨力（有裕度、效应 > 噪声底）。**
- **红绿变异试验要"值级"而非"功能级"**：把整条路堵死只能证明"路堵了会失败"；
  本版用的四个变异体 = 路由退回旧结构 / 去掉一次性账本判据 / AV 容量 10→0 / 形状定义漏一个键，
  各自**精准命中**对应断言且不牵连无关段落。
- **`?v=` 必须随发布刷新**（`index.html` 全量）：不刷新则老访客按缓存加载旧 JS，
  本版新增的 UI 全部不可见（2026-09-18 整改前的实际状态）。
