...
- **凡"要发起一次出击 / 进击 / prepareBattle"的断言，先把舰队耐久补满再断言**。
  ...
  （V0.305 实测：`startHardWave 发起第二波…` 与 `收兵后仍可再次发起强敌阶` 两条，各 ~2.5%/跑次）。
  补满耐久只消除噪声路径：若真有状态机 bug，守卫之外的判据仍会变红。
...
- **效度抽检工具**：`npm run probe:recon`（`scripts/probe_recon_variance.js`）——
  ...
  参考基线：差值 **4.99 ± 0.62pp**（16 轮 ×4000 场），2.5pp 阈值余量 ≈4σ。
- **⚠️「12 连跑全绿」不能证明没有 flaky**：约 5%/跑次的假失败，在 12 次里有 ~46% 概率恰好全绿。
  V0.304 的"12 连跑全绿"就是这么被蒙过去的（V0.305 用 30/40/60 轮 soak 才把它们揪出来）。
  **加完机制后至少跑 40 轮 soak**，并把失败轮的 ✗ 行落盘（崩溃时进程不留汇总行，只看 exit code 归不了因）。
  待办：把 soak 固化为 `npm run sim:soak [轮数]`。
