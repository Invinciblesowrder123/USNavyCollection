...
- 装备字段：`id, en, zh, cat(主炮小/主炮中/主炮大/副炮/鱼雷/舰战/舰攻/舰爆/水侦/水爆/电探/高角炮/机枪/声呐/爆雷/穿甲弹/设备), stat{fp,tp,aa,arm,asw,los,evd,bmb,avg,radius,speed}, slotType, cost, buildable`
- 制空值 = 对空值 × √搭载数（截尾取整）
...
- 资源：油/弹/钢每3分钟+3，铝每3分钟+1；上限随提督等级
...
**v0.1 完成（2026-08-03）**：完整核心循环可玩。
- 引擎：255 项断言全过（`npm run sim`）；浏览器 E2E 39 项全过（`public/test_flow.html`）
- 内容：55舰/44装备/3海域/8远征/30任务/5演习对手/57张SVG立绘
- 分支：`main`（v0.1 稳定）+ `develop`（开发中）
- 已知平衡说明：满级舰队打 1-3 BOSS 胜率 ~100%（正常，早期舰队打不过属预期）

**迭代候选（未做）**：装备改修工厂 / 活动海域(贴条/削甲/友军) / 图鉴 / 音效 / 更多海域。
