...
const Battle = (() => {
  ...
  function aaShootdown(attackers, defenders, isMySide, aaBonus, log) {
    ...
    for (const a of attackers) {
      ...
      for (const sl of a.slots) {
        ...
        if (Util.chance(0.5)) down += Math.floor((wAA + fleetAA) * 0.25 * (isMySide ? 0.8 : 0.75));
        let ci = 0;
        ...
        sl.size = Math.max(0, sl.size - down);
        total += down;
      }
    }
    return total;
  ...
  /* ============ 开幕空袭（wiki：由舰攻/舰爆/水爆实施；机种倍率 舰攻0.8或1.5、舰爆1.0；命中70~80%不受疲劳/阵型影响） ============ */
  function airStrike(log, attackers, defenders, sideLabel, snap, defSide, defForm) {
  ...
  function battle(fleetA, fleetB, formationA, formationB, opts = {}) {
    ...
    if ((myAir > 0 || enAir > 0)) {
      if (losOk) {
        ...
        /* 开幕空袭（剩余舰攻/舰爆/水爆） */
        airStrike(log, sideA, sideB, '我军', pushSnap, sideB, formBName);
        airStrike(log, sideB, sideA, '敌军', pushSnap, sideA, formAName);
      } else {
        L('索敌失败！无法参加航空战，制空权自动丧失！');
        /* 我方舰载机不离舰不参与航空战；敌方空袭仍会被我方对空炮火迎击 */
        aaShootdown(sideB, sideA, true, fA.aa, log);
        airStrike(log, sideB, sideA, '敌军', pushSnap, sideA, formAName);
      }
    } else {
      L('双方均无航空战力，不发生航空战。');
    ...
    const openTorp = (s, targetSide, defForm) => {
      ...
      if (Math.random() > ch) { L(`开幕雷击！${s.name} 的鱼雷未命中。`); return; }
